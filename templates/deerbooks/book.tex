...
\title{% templatetag openbrace %}\Huge {% if toc.title %}{{toc.title|safe}}{% else %}Untitled{% endif %}{% templatetag closebrace %}
...
\copyright {{timestamp|date:'Y'}} by {% include 'authorname.txt' %}, and is free for distribution, but may not be modified or used for monetary gain.  Original version and further downloads available at \url{% templatetag openbrace %}{{source_url}}{% templatetag closebrace %}

\tableofcontents

{% for page in pages %}
\chapter{% templatetag openbrace %}{% if page.title %}{{page.title|safe}}{% else %}Untitled{% endif %}{% templatetag closebrace %}
{% html_tex page.body promote=False %}
{% endfor %}
